...
void main()
{
    ...
    vec3 V = normalize(vec3(eyePosition) - ECPosition);
    ...
    vec3 L1 = normalize(lightPosition1 - ECPosition1);
       // L1 = normalize(vec3(1.2, 0, -1));
    vec3 V1 = normalize(vec3(eyePosition) - ECPosition1);
    ...
    vec4 specular1 = specularColor1 * pow(max(dot(N1 , H1) , 0.0) , shininess)*specularColorm;
        test =normalize(normalMatrix * normal);
    specularOut = specular*0.4+specular1;
