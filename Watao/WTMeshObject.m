...
-(GLKMatrix4)moveRotateX:(float) rotate rotatex:(float) rotatex scale: (GLKVector3) scale translate:(GLKVector3)translate base:(GLKMatrix4)baseModelViewMatrix project:(GLKMatrix4) projectionMatrix{
    ...
    GLKMatrix4 modelViewMatrix = GLKMatrix4MakeTranslation(translate.x, translate.y, translate.z);
    //at last tranlsate
    modelViewMatrix = GLKMatrix4RotateX(modelViewMatrix, rotatex);
    modelViewMatrix = GLKMatrix4RotateY(modelViewMatrix, rotate);
...
- (GLuint)setupTexture:(NSString *)fileName {
    ...
    //if (tt >= 0) glDeleteTextures(1, &textureID);
    NSLog(@"tt: %d", tt);
    GLuint texName;
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE);//new year
...
//            float  test = [[verticeLines objectAtIndex: ind*3] floatValue];
            ...
            {
               // NSLog(@"table");
            _vertices[i].coord = GLKVector4Make([[[verticeLines objectAtIndex: ind] objectAtIndex:0] floatValue]*0.7f, [[[verticeLines objectAtIndex: ind] objectAtIndex:1] floatValue]*0.4f, [[[verticeLines objectAtIndex: ind] objectAtIndex:2] floatValue]*0.7f, 1.0f);
            ind = [[normalIndiceLines objectAtIndex:i] intValue]-1;
            }
            else if([name isEqualToString:@"shadow"])
            {
                _vertices[i].coord = GLKVector4Make([[[verticeLines objectAtIndex: ind] objectAtIndex:0] floatValue]*0.6f, [[[verticeLines objectAtIndex: ind] objectAtIndex:1] floatValue]*0.4f, [[[verticeLines objectAtIndex: ind] objectAtIndex:2] floatValue]*0.6f, 1.0f);
                ind = [[normalIndiceLines objectAtIndex:i] intValue]-1;
            ...
            _vertices[i].normal = GLKVector4Make([[[normalLines objectAtIndex: ind] objectAtIndex:0] floatValue], [[[normalLines objectAtIndex: ind] objectAtIndex:1] floatValue], [[[normalLines objectAtIndex: ind]objectAtIndex:2] floatValue], 1.0f);            ind = [[textureIndiceLines objectAtIndex:i] intValue]-1;
            float i1 = [[[textureLines objectAtIndex: ind] objectAtIndex:0] floatValue];
            float i2 = [[[textureLines objectAtIndex: ind] objectAtIndex:1] floatValue];
            NSLog(@"i1: %f i2: %f", i1,i2);
            for  (;i1 < 0;) {i1 += 1.0f; }
            for  (;i2 < 0;) {i2 += 1.0f; }
            for  (;i1 > 1;) {i1 -= 1.0f; }
            for  (;i2 > 1;) {i2 -= 1.0f; }
            _vertices[i].textureCoords  = GLKVector2Make(i1, i2);
