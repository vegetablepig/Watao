...
//    float lastRandomDeltaR = 0.3f*(WT_NUM_LEVEL)/WT_NUM_LEVEL+0.1;
    ...
    NSLog(@"randA: %f randB: %f", randA, randB);
    for(int i = 0; i < WT_NUM_LEVEL; i++){
...
//            lastRandomDeltaR = randomDeltaR;
        ...
        for(int j = 0; j < WT_NUM_VERTEX; j++){
            ...
            //NSLog();
            _vertices[i*WT_NUM_VERTEX+j].textureCoords = GLKVector2Make((float)j*1.0f/(float)vertexMinusOne/2.0f, i*1.0f/(float)WT_NUM_LEVEL);
            ...
            //fix it!
            _vertices[WT_NUM_LEVEL*WT_NUM_VERTEX+i*WT_NUM_VERTEX+j].textureCoords = GLKVector2Make(0.5+(float)j*1.0f/(float)vertexMinusOne/2.0f, i*1.0f/(float)WT_NUM_LEVEL);
            //NSLog();
...
-(void)calNormalY{
    ...
    for (int i = 0; i < WT_NUM_LEVEL; i++) {
        for(int j = 0; j < WT_NUM_VERTEX; j++){//j = 1?
            ...
            if (i > 0) pDown = _vertices[(i-1)*WT_NUM_VERTEX+j].coord; //**i > 0 else halt//
//            pLeft = _vertices[i*WT_NUM_VERTEX+(j+WT_NUM_VERTEX-1)%WT_NUM_VERTEX].coord;
//            pRight = _vertices[i*WT_NUM_VERTEX+(j+WT_NUM_VERTEX+1)%WT_NUM_VERTEX].coord;
            //
...
- (GLuint)setupTexture:(NSString *)fileName {
    ...
    CGImageRef spriteImage = [self addImage:image toImage:image].CGImage;
    if (!spriteImage) {
